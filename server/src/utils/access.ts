...
const checkOtherAccess = async (access: AccessRepository, request: OtherAccessRequest): Promise<Set<string>> => {
  ...
  switch (permission) {
    ...
    case Permission.FolderUserCreate:
    case Permission.FolderUserUpdate:
    case Permission.FolderUserDelete: {
      const isOwner = await access.folder.checkOwnerAccess(auth.user.id, ids);
      const isShared = await access.folder.checkSharedFolderAccess(
        auth.user.id,
        setDifference(ids, isOwner),
    ...
    case Permission.FolderDelete: {
      return access.folder.checkOwnerAccess(auth.user.id, ids);
    }
